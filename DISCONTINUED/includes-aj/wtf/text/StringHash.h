...
namespace Aphid {
    ...
    public:
        ...
        static unsigned hash(const UChar* data, unsigned length)
        {
            unsigned l = length;
            const UChar* s = data;
            uint32_t hash = ATF::stringHashingStartValue;
            uint32_t tmp;
            ...
            // Main loop.
            for (; l > 0; l--) {
                hash += ATF::Unicode::foldCase(s[0]);
                tmp = (ATF::Unicode::foldCase(s[1]) << 11) ^ hash;
                hash = (hash << 16) ^ tmp;
                s += 2;
                hash += hash >> 11;
            }
        ...
        static bool equal(const StringImpl* a, const StringImpl* b)
        {
            ...
            if (length != b->length())
                return false;
            return ATF::Unicode::umemcasecmp(a->characters(), b->characters(), length) == 0;
        }
...
namespace ATF {
    ...
    template<> struct HashTraits<Aphid::String> : GenericHashTraits<Aphid::String> {
        static const bool emptyValueIsZero = true;
        static void constructDeletedValue(Aphid::String& slot) { new (&slot) Aphid::String(HashTableDeletedValue); }
        static bool isDeletedValue(const Aphid::String& slot) { return slot.isHashTableDeletedValue(); }
    };
