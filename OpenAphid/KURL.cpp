...
namespace Aphid {
  ...
  {
	  return characterClassTable[static_cast<unsigned char>(c)] & SchemeFirstChar;
  }

  static inline bool isSchemeFirstChar(UChar c)
  {
	  return c <= 0xff && (characterClassTable[c] & SchemeFirstChar);
  }

  static inline bool isSchemeChar(char c)
  {
	  return characterClassTable[static_cast<unsigned char>(c)] & SchemeChar;
  }

  static inline bool isSchemeChar(UChar c)
  {
	  return c <= 0xff && (characterClassTable[c] & SchemeChar);
  }

  static inline bool isUserInfoChar(unsigned char c)
  {
	  return characterClassTable[c] & UserInfoChar;
  }

  static inline bool isHostnameChar(unsigned char c)
  {
	  return characterClassTable[c] & HostnameChar;
  }

  static inline bool isIPv6Char(unsigned char c)
  {
	  return characterClassTable[c] & IPv6Char;
  }

  static inline bool isPathSegmentEndChar(char c)
  {
	  return characterClassTable[static_cast<unsigned char>(c)] & PathSegmentEndChar;
  }

  static inline bool isPathSegmentEndChar(UChar c)
  {
	  return c <= 0xff && (characterClassTable[c] & PathSegmentEndChar);
  }

  static inline bool isBadChar(unsigned char c)
  {
	  return characterClassTable[c] & BadChar;
  }
  ...
  {
	  return Aphid::protocolIs(string, protocol);
  }
  ...
  void KURL::init(const KURL& base, const String& relative, const TextEncoding& encoding)
  {
	  ...
	  String rel = relative;
	  if (rel.contains('\\') && !(protocolIsAJ(rel) || protocolIs(rel, "data")))
		  rel = substituteBackslashes(rel);

	  String *originalString = &rel;

	  bool allASCII = Aphid::charactersAreAllASCII(rel.characters(), rel.length());
  ...
  unsigned short KURL::port() const
  {
	  ...
	  bool ok = false;
	  unsigned number = Aphid::charactersToUIntStrict(stringData + m_hostEnd + 1, m_portEnd - m_hostEnd - 1, &ok);
  ...
  bool KURL::protocolIs(const char *protocol) const
  {
	  ...
	  // AJ URLs are "valid" and should be executed even if KURL decides they are invalid.
	  // The free function protocolIsAJ() should be used instead.
  ...
  static void appendEncodedHostname(UCharBuffer& buffer, const UChar *str, unsigned strLen)
  {
	  ...
	  if (strLen > hostnameBufferLength || Aphid::charactersAreAllASCII(str, strLen)) {
  ...
  static void encodeRelativeString(const String& rel, const TextEncoding& encoding, CharBuffer& output)
  {
	  ...
	  int pathEnd = -1;
	  if (encoding != pathEncoding && encoding.isValid() && !protocolIs(rel, "mailto") && !protocolIs(rel, "data") && !protocolIsAJ(rel)) {
  ...
  bool isDefaultPortForProtocol(unsigned short port, const String& protocol)
  {
	  ...
	  typedef HashMap<String, unsigned, Aphid::CaseFoldingHash> DefaultPortsMap;
