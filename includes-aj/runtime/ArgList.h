...
namespace AJ {
    ...
    class ArgList {
        friend class JIT;
    public:
        typedef AJValue* iterator;
        typedef const AJValue* const_iterator;
        ...
        ArgList(AJValue* args, unsigned argCount)
            : m_args(args)
            , m_argCount(argCount)
        {
#if ENABLE(JSC_ZOMBIES)
        ...
        ArgList(Register* args, int argCount)
            : m_args(reinterpret_cast<AJValue*>(args))
            , m_argCount(argCount)
        {
            ASSERT(argCount >= 0);
        }
        ...
        ArgList(const MarkedArgumentBuffer& args)
            : m_args(reinterpret_cast<AJValue*>(const_cast<Register*>(args.begin())))
            , m_argCount(args.size())
        {
        }

        AJValue at(size_t idx) const
        {
            if (idx < m_argCount)
                return m_args[idx];
            return jsUndefined();
