...
{
    // AJ doesn't recognize fractions of a millisecond.
    return floor(ATF::currentTimeMS());
}
...
using ATF::dateToDaysFrom1970;
using ATF::dayInMonthFromDayInYear;
using ATF::dayInYear;
using ATF::minutesPerHour;
using ATF::monthFromDayInYear;
using ATF::msPerDay;
using ATF::msPerSecond;
using ATF::msToYear;
using ATF::secondsPerMinute;
...
#else
        utcOffset = static_cast<int>(getUTCOffset(exec) / ATF::msPerSecond + (isDST ? ATF::secondsPerHour : 0));
#endif
